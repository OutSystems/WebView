...
function getFunctionReturnType(func) {
    if (func.returnType.parameters) {
        return func.returnType.parameters[0].name;
    }
    ...
    function DummyTemplate(context) {
        ...
        this.dumpModule = function (module) {
            return module.classes.map(_this.dumpClass).join("\n") + "\n" +
                module.interfaces.map(_this.dumpInterface).join("\n") + "\n" +
        ...
        this.dumpInterface = function (interf) {
            return "INTERFACE " + interf.name + _this.context["extraInfo1"] + " {\n" +
                interf.functions.map(_this.dumpMethod).join("\n") +
                "\n}";
        };
